...
- Thrive-only uploads now stage a renamed temp copy before the hotfolder copy lands, using Fiery's Smart File Name Job Submission pattern: `OriginalFile_#JMD#_copies;magnification;rotation;order;company;customer.ext`.
- The parser understands that `_#JMD#_` metadata format, so Thrive jobs can still recover copies, magnification, rotation, work order, company, and customer fields from the hotfolder copy.
...
- The Fiery-native import artifact is `/rip-queue/fiery/media-catalog?format=xml`.
