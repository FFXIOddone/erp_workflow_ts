...
export function FileSharesModal({ isOpen, onClose, equipmentId, equipmentName, ipAddress }: FileSharesModalProps) {
  ...
  return (
    <EquipmentModalShell
      ...
      maxWidthClassName="max-w-3xl max-h-[80vh]"
    >
      {isLoading ? (
            ...
            <div key={share.name} className="border border-gray-200 rounded-lg overflow-hidden">
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <svg
                    className={`h-4 w-4 text-gray-400 transition-transform ${expandedShare === share.name ? 'rotate-180' : ''}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
...
export function WinServicesModal({ isOpen, onClose, equipmentId, equipmentName, ipAddress }: WinServicesModalProps) {
  ...
  return (
    <EquipmentModalShell
      ...
      maxWidthClassName="max-w-4xl max-h-[85vh]"
      bodyClassName="p-0"
    >
      <div className="flex items-center gap-3 px-5 py-3 border-b border-gray-100 bg-gray-50">
        <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden">
          {(['running', 'stopped', 'all'] as const).map((f) => (
            ...
            >
              {f === 'running'
                ? `Running${summary ? ` (${summary.running})` : ''}`
                : f === 'stopped'
                  ? `Stopped${summary ? ` (${summary.stopped})` : ''}`
